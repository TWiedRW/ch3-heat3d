...
PARAGRAPH ON DIFFERENCE IN SEEING BETWEEN THE CHART TYPES.
...
We denote "stimuli" to represent the magnitude of our chosen values. In a 3D Cartesian space, the X- and Y- axes represent the coordinates of the stimuli, and the Z-axis represents the value for the stimuli. Each X and Y coordinate is represented by a square tile with a 1:1 aspect ratio. In a 2D space, the Z variable is mapped to fill with a monochromatic gradient. All stimuli and remaining randomly generated values range between 0 and 100 units. In this experiment, $X=1, 2, \dots,10$ and $Y=1, 2,\dots,10$.
...
![Data Set 1](images/3dp-gradient-set1.JPG){#fig-3dp-gradient}

![Data Set 2](images/3dp-gradient-set2.JPG){#fig-3dp-solid}
:::
...
```{r}
library(flexmix)
fm <- stepFlexmix(user_slider ~ 0 + target_ratio:media:set | user_id,
        data = res_q2_filtered, k = 1:10)

summary(fm)
parameters(getModel(fm, which = 'BIC'))

res_q2 %>% 
  mutate(diff_ratio = user_slider - target_ratio,
         diff_diff = user_slider - target_diff) %>% 
  ggplot(mapping = aes(x = diff_ratio, y = diff_diff, color = factor(pair_id))) + 
  geom_point()
