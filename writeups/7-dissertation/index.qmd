...
### Addressing Random Continuous Responses

A common technique in psychology experiments to address careless respondents is to fit mixture models [@ward2023]. Since some of our participants may have used alternative estimation strategies, we consider a mixture model using the `flexmix` package (XXX) that fits the linear relationship between the submitted slider position and the true ratio for each chart type and underlying data set. We also assume that participants do not change strategies throughout the experiment. 
...
library(flexmix)
set.seed(4124)
sfm <- stepFlexmix(user_slider ~ 0 + target_ratio:media:set | user_id:block, k = 1:10, nrep = 5,
            data = filter(res_q2_filtered, pair_id != 5 & user_larger==true_larger))
fm <- getModel(sfm, which = 'BIC')
fm <- stepFlexmix(user_slider ~ 0 + target_ratio:media:set | user_id:block,
                  k = 12, data = res_q2_filtered)
parameters(fm)
summary(fm)
```
...
res_q2_filtered %>% 
  group_by(user_id) %>% 
  mutate(error = scale(user_slider-target_diff, center = T, scale = F)) %>% 
  ggplot(mapping = aes(x = error, group = user_id)) + 
  geom_density(color = alpha('black', alpha = 1/5))
```
...
res_q2_filtered %>% 
 ungroup() %>% 
 mutate(cluster = clusters(fm)) %>% 
 ggplot(mapping = aes(x = user_trial_order, y = user_slider, color = factor(cluster))) + 
 geom_line() + 
 geom_line(aes(y = target_ratio), color = 'black', linetype = 'dashed') +
 geom_line(aes(y = target_diff), color = 'blue', linetype = 'dashed') +
 facet_wrap(~user_id)
```
...
```{r}
sfm <- stepFlexmix(user_slider ~ 0 + target_ratio  | user_id:block:user_set_order,
            k = 1:10, data = res_q2_filtered)
plot(sfm)
fm <- flexmix(user_slider ~ 0 + target_ratio  | user_id:block:user_set_order,
            k = 5, data = res_q2_filtered)
parameters(fm)
summary(fm)
```
...
res_q2_filtered %>% 
  ...
  slice_sample(n = 20) %>% 
  unnest(data) %>% 
  ggplot(mapping = aes(x = user_trial_order, y = user_slider)) + 
  geom_point(aes(color = factor(cluster))) + 
  geom_line(aes(color = factor(cluster), group = user_id)) + 
  geom_line(aes(y = target_ratio), linetype = 'dashed') + 
  facet_wrap(~user_id)
...
```{r}
tmp <- res_q2_filtered %>% 
  mutate(error_cm = log2(abs(user_slider - target_diff) + 1/8)) %>% 
  group_by(user_id) %>% 
  nest() %>% 
  ungroup() %>% 
  # slice_sample(n = 20) %>% 
  unnest(data) %>% 
  group_by(user_id) %>% 
  mutate(error_scaled = scale(error_cm, center = T, scale = F))
...
tmp %>% 
  ...
  slice_sample(n = 20) %>%
  unnest(data) %>% 
  ggplot(mapping = aes(x = user_trial_order, y = error_scaled)) + 
  geom_point() + 
  geom_line() + 
...
res_q2_filtered %>% 
  ...
  geom_line() +
  geom_line(aes(y = target_ratio), linetype = 'dashed', alpha = 2/3) + 
  facet_wrap(~user_id)
```
...
i <- 1
p <- res_q2_filtered %>% 
  # filter(user_id == (res_q2_filtered$user_id)[i]) %>% 
  ggplot(mapping = aes(x = pair_id, y = q2_error_cm)) + 
  geom_point() + 
  # geom_point(aes(y = target_ratio), shape = 2, color = 'red') +
  facet_wrap(~set+media) + 
  labs(title = res_q2_filtered$user_id[i]) + 
  theme_bw()
print(p)



mod <- lmer(user_slider ~ set*media*pair_id + (1 | user_id:block/set:media),
...
```{r}
ggplot(res_q2_filtered, aes(x = pair_id, y = user_slider)) + 
  geom_boxplot(fill = '#b8b8b8') + 
  geom_line(aes(group = user_id), alpha = 1/10) + 
  geom_point(aes(y = target_ratio), color = 'red', shape = 2) +
  facet_wrap(~set + media) + 
  theme_bw()
```
...
```{r}
mod <- lmer(q2_error_cm ~ set*media*pair_id + (1|block:user_id/set:media),
     data = filter(res_q2_filtered, pair_id != 5 & (user_larger == true_larger)))
...
library(emmeans)
em <- emmeans(mod, ~media|pair_id+set)
data.frame(pairs(em)) %>% filter(p.value < 0.1)
```

```{r}
em %>% 
  data.frame() %>% 
  ggplot(mapping = aes(x = emmean, y = media, group = set)) + 
  geom_point(position = position_dodge(width = 2/3)) + 
  geom_errorbar(aes(xmin = lower.CL, xmax = upper.CL, linetype = set),
                position = position_dodge(width = 2/3), width = 1/4) +
  facet_wrap(~pair_id) + 
...
for(i in 1:1000) {
  tmp_sample <- sample(unique(res_q2_filtered$user_id), replace = T)
  
  tmp_res <- res_q2_filtered %>% 
    # inner_join(tmp_sample, relationship = 'many-to-many') %>% 
    filter(user_id %in% tmp_sample) %>% 
    group_by(media
             , set, pair_id
             ) %>% 
    summarize(midmean = mean(q2_error_cm, trim = 0.25), .groups = 'keep') %>% 
    mutate(sample = i)
  df_bootstrap <- bind_rows(df_bootstrap, tmp_res)
}
```

```{r}
df_bootstrap %>% 
  ggplot(mapping = aes(x = midmean, y = media)) + 
  # geom_point() + 
  geom_violin() +
  facet_wrap(~pair_id) +
  theme_bw() + 
