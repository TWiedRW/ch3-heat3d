...
Our initial exploratory data analysis of Question 2 revealed an interesting comprehension issue. While participants were tasked to estimate the size of the smaller value if the larger value in the pair represented 100 units, a non-negligible amount of participants had alternative estimation strategies. This was observed predominately in stimuli pair 5, which was where both values were the same. Based on consistent trends with possible strategies, we suspect that some participants either estimated the difference between the values in the stimuli pair or the value of the smaller value with respect to the entire chart. 
