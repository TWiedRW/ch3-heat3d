...
The first question asks participants which value in the pair of stimuli is larger, giving the two labels on the stimuli or specifying that they are the same height.
...
res_all %>% 
  ...
  ggplot(mapping = aes(x = factor(pair_id), y = user_slider)) + 
...
```{r}
res <- res_all %>% 
  filter(pair_id != 5 & q1_correct)

shape_est <- (mean(res$q2_abs_error)^2) / var(res$q2_abs_error)
rate_est <- mean(res$q2_abs_error) / var(res$q2_abs_error)

res %>% 
  ggplot(mapping = aes(x = q2_abs_error)) + 
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, color = 'black', fill = 'skyblue') + 
  stat_function(fun = dgamma,
                args = list(shape = shape_est, rate = rate_est),
                color = 'red') + 
  facet_wrap(~q1_correct, scales = 'free_y')
```
...
res_all %>% 
  filter(q1_correct & pair_id != 5) %>% 
  group_by(user_id, block) %>% 
  filter(mean(q2_abs_error) > 50) %>% 
  mutate(time = end_time - start_time) %>% 
  ggplot(mapping = aes(x = time, y = q2_signed_error)) + 
