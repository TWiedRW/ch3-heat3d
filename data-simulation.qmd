...
#| eval: false
#| echo: false
# bibd <- takeout(menu_bibd(t = 10, r = 2, k = 5))
# examine_recipe(bibd)
# examine_recipe(takeout(menu_lsd(t=5)))
...
lr <- design("Latin Rectangle Design") %>%
  set_units(ratio = 5,
            vars = 10,
            unit = crossed_by(ratio, vars)) %>%
  set_trts(block = 5) %>%
  allot_trts(block ~ unit) %>%
  assign_trts("random", seed = 195)
